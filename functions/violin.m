...
for i=1:size(Y,2)
    ...
    elseif isempty(b)
        [f, u, bb]=ksdensity(Y{i},0:1:max(Y{i})+1,'support','positive','bandwidth',.1);
    end
...
for i=i:size(Y,2)
    if isempty(lc) == 1
        ...
        if setX == 0
          fill([F{i}+i;flipud(i-F{i})],[U{i};flipud(U{i})],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc,'LineWidth',3,'FaceColor',lc);
        else
            h(i)=fill([F{i}+x(i);flipud(x(i)-F{i})],[U{i};flipud(U{i})],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc,'LineWidth',3,'FaceColor',lc);
        end
    end
    hold on
    % if setX == 0
